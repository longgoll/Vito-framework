...
- [ ] **Native `/metrics` Endpoint**: Tự động thu thập và xuất các chỉ số chuẩn Prometheus:
...
- [ ] **OpenTelemetry (OTel) Tracing Integration**: Tự động truyền dẫn Trace ID và Span ID qua HTTP Headers (`traceparent`).

### 2. 🧪 Testing Framework & Mock Suite (`vito/testing`)
- [ ] **Enhanced `app.inject()`**: Mở rộng khả năng giả lập HTTP request siêu tốc không cần cổng TCP thực tế với đầy đủ Body, Headers, Cookies.
- [ ] **DB Test Fixtures & Isolation**: Hỗ trợ tự động Rollback CSDL sau mỗi lượt Test để đảm bảo môi trường Test sạch sẽ.

### 3. 🛠 CLI Code Generators & DX Tooling (`vit` CLI)
- [ ] **`vit generate` (alias `vit g`)**: Tự động sinh boilerplate code cho Controller, Service, Middleware, Model, Migration.
- [ ] **Interactive CLI Diagnostics**: Lệnh `vit doctor` kiểm tra cấu hình môi trường, phiên bản CSDL và các dependency.
